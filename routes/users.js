...
router.post('/shipping-address', (req, res, next) => {
	...
	User.clearDefaultAddress(data, (err) => {
		if(err) {
			res.json({ success: false, msg: "The default address could not be cleared"})
		}
		else {
			// Pushing the new address
			User.addShippingAddress(req.body, (err) => {
				if(err) {
					res.json({ success: false, msg: "The new shipping address could not be added"})
				}
...
router.delete('/shipping-address/:idUser/:idShippingAddress', async(req, res, next) => {
	User.delShippingAddress(req.params, (err) => {
		if (err) {
			return res.json({ success: false, msg: "The shipping address could not be deleted"})
		}
		return res.json({ success: true });
	});
});
